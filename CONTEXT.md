...
- **File drop** path quoting is **POSIX-only** today. A **shell-mode PTY** drop single-quotes paths with spaces/special chars — correct for bash/zsh/fish, but wrong on Windows (cmd.exe treats single quotes literally; PowerShell rejects the `'\''` escape), and a Windows path's backslashes always force the (wrong) quoting branch. The **agent-mode** raw path may likewise need per-agent `\` escaping on Windows. Tracked as `TODO(windows)` in `lib/fileDrop.ts`; the proper fix is to plumb the spawned shell kind through and branch the quoting per shell before the Windows ship.
