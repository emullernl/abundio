...
- Copilot's **Waiting** is ambiguous at request time. Copilot fires a `permissionRequest` hook for permission-gated tools (edits/writes; read-kind tools like `glob`/`view` short-circuit it) — even when auto-approved — in the order `preToolUse → permissionRequest → postToolUse`, and the only "tool finished/unblocked" signal (`postToolUse`) arrives after the tool runs. So `permissionRequest` alone does not mean the Agent is blocked. Resolved operationally: for **Copilot** a tool counts as **Waiting** only once its `permissionRequest` has gone 1500ms with no matching `postToolUse` *and* the pane's output has stayed quiet (a still-running tool keeps emitting output, which re-arms the timer); until then the dot stays **Working**. Other Agents enter **Waiting** immediately on a permission/input hook — theirs fires only on a genuine prompt. See ADR-0015.
