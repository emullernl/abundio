...
export function TerminalSlot({
	...
	useEffect(() => {
		...
		const handler = (e: MouseEvent) => {
			// Swallowed unconditionally, including in a mouse-reporting pane where
			// we are NOT going to open our own menu. This is not about our menu:
			// xterm's own contextmenu listener moves its hidden textarea under the
			// cursor, which on Windows WebView2 pastes the clipboard straight into
			// the PTY.
			e.preventDefault();
			e.stopPropagation();
			handleFocus();
	...
	// Withhold bare pointer MOVEMENT from a mouse-reporting app while a selection
	// is up. DECSET 1003 ("any event") reports every move, not just clicks — and
	// each report goes out through `triggerDataEvent(report, true)`, where the
	// `true` marks it user input, which makes xterm's SelectionService clear the
	// selection. So merely moving the mouse would wipe a selection the user was
	// on their way to copy.
	//
	// That walk is now the long one. The right button belongs to the program
	// (ADR-0031), so Copy is reached from the pane menu button in the title bar
	// — and the title bar renders INSIDE this container, so the whole journey is
	// covered by this one capture-phase listener.
	//
	// The cost is that a TUI loses hover feedback until the selection is dropped
	// (any click does that). Gated on `buttons === 0` so an in-progress drag is
	// never touched — that is how the selection gets extended.
	//
	// Stands down unless the app is actually reporting the mouse. In a pane with
	// no mouse tracking xterm sends nothing on movement, so there is no selection
	// at risk and the guard would be pure cost: `stopPropagation()` on the
	// container keeps the event from the `.xterm-screen` descendant, where
	// xterm's Linkifier listens for the hover that underlines URLs and file
	// links.
	useEffect(() => {
		const el = containerRef.current;
		if (!el) return;
