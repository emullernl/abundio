...
- **`emit_to(label, …)` does not isolate a global `listen`.** The JS `listen` from `lib/ipc.ts` has target `Any`, and Tauri delivers *every* event to an `Any` listener whatever the emit's target — so an event Rust aims at one Window (`emit_to_focused`, `pr-changes`, `update-available`, `whats-new`) reaches every Window. Hear those through `listenToThisWindow`, which listens on the current webview. This is how File → Switch Profile once raised the confirm dialog in every Window. A Rust "send to one Window" helper must also never fall back to `app.emit`, which is a broadcast.
