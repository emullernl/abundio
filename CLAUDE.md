...
- In a **mouse-reporting pane** (a TUI with DECSET 9/1000/1002/1003 on — Copilot CLI does, Claude Code doesn't), xterm disables its own selection service, so **Option+drag** (macOS) / **Shift+drag** (Win/Linux) is the only way to select text. The right button never reaches the program — it opens our context menu, whose *Send Right Click to Terminal* item forwards that one click for TUIs that want it. Both guards are inert in panes with no mouse reporting, or they would starve xterm's Linkifier of hover. See the `Mouse-reporting pane` entry in `CONTEXT.md`.
