...
- **Focus sweep:** an accent-coloured arc that travels once clockwise around the pane border (~450 ms) and fades. It fires on every change of Focused pane to a different pane, except the first focus after a Window opens. A new focus cuts the previous sweep short. Under `prefers-reduced-motion` it is a static fading border. Drawn in a `pointer-events: none` overlay above the pane. Global setting `focusSweep`, default on, in Settings ▸ Theme.
