...
- **Restored scrollback is history, not a request.** `@xterm/addon-serialize` writes mouse DECSETs into a snapshot, so a pane replaying its last session would otherwise record the dead program's modes, raise a badge over a freshly spawned shell, and — on a click — replay `?1003h` into that shell, which would then take raw reports on every click and movement. Both hooks stand down while `restoring` is set, and the DECSET is swallowed regardless of the pane's answer.
