...
An answer that agrees with the setting is not recorded as an override at all. Otherwise clicking the badge twice would leave the pane looking untouched while holding an override equal to the global value, and every later change in Settings would skip it, permanently and invisibly. Flipping it back means what it looks like.
...
For the same reason the override lives in a module-level map keyed by pane, **not on the `ManagedTerminal`**. The pane outlives its terminal: switching workspaces disposes the xterm instance while the PTY keeps running under a background tracker (ADR-0020), and coming back to find the program's mouse silently revoked would be the opposite of what the click meant. `teardownTerminal` drops it, because there the pane is really gone.

The badge's click flips **what the badge shows**, not what the pane's stored answer says. Those differ exactly where a mixed DECSET has failed open — the pane's answer is still "blocked" while xterm is plainly reporting — and the label the user just read says *Click to block it here*. Deriving the click from the stored answer would allow instead, and the sweep the badge exists to perform would be unreachable.
...
- **A dead session's scrollback is history; a live PTY's log is not.** `@xterm/addon-serialize` writes mouse DECSETs into a snapshot, so a pane replaying its last session would otherwise record the dead program's modes, raise a badge over a freshly spawned shell, and — on a click — replay `?1003h` into that shell, which would then take raw reports on every click and movement. Both hooks therefore stand down while replaying history, and the DECSET is swallowed regardless of the pane's answer.

  The distinction matters more than it looks, because `restoring` alone does not draw it. A workspace switch-away disposes the xterm instance while the PTY keeps running (ADR-0020), and switching back replays that pane's **raw log** to a program that is still there and still expects the mouse. Treating that as history would drop the badge — silently removing the only control that could hand the mouse back — and, in a pane the user had deliberately allowed, lose mouse reporting outright. `restoreIsLive` carries `loadScrollback`'s own discriminator through to the hooks, which treat a live replay as ordinary output arriving late. Set/reset ordering in the log converges on the state the program is actually in.

  Replayed DECSETs are swallowed under the same all-or-nothing rule as live ones. The snapshot is ours and emits one mode per sequence, but a raw log is arbitrary program output, and dropping an `\e[?1049;1002h` whole would take the alternate-screen switch with it — the same mess the live path fails open to avoid.
