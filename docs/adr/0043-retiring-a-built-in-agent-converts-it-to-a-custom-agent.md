...
- **A one-shot prune after the first non-empty `$PATH` scan.** ADR-0037 left users from before Agent seeding with every built-in Watched, so "Watched" does not imply "Installed". After the startup scan, `pruneRetiredBuiltins` removes every marked Agent whose command is not Installed and clears the marker on the ones that are. An empty scan is a failed scan and changes nothing. Because this deletes rather than toggles, it also runs only when the scan used the real login-shell `$PATH` (`agents_path_is_resolved`): when the shell times out, the fallback `$PATH` can find Homebrew agents yet miss one in `~/.local/bin`, where Aider is usually installed. This runs on every launch until nothing carries the marker, independent of the one-time seeding claim.
