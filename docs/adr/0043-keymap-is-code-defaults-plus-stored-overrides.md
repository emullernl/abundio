...
- **Open settings is the only Shortcut synced to the native menu.** Its Chord is sent to Rust (`set_settings_accelerator`), which rebuilds the menu, so the Settings… item shows the chord that works and a rebind frees Cmd+,. The other menu chords are **reserved** and recording one is refused: Quit, Minimize and the Edit menu's undo/redo/cut/copy/paste/select all on every platform (the Edit and Window menus exist everywhere, and muda gives their items Ctrl accelerators on Windows/Linux, with Redo as Ctrl+Y), plus Hide, Hide Others and Enter Full Screen on macOS. Close Window's ⌘W / Ctrl+W is not reserved: it is Close tab's default. Rust validates the accelerator string at the IPC boundary, since an invalid one would make `build_menu` fail and leave the menu stale.
