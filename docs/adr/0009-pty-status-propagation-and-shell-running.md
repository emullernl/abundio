...
> **Update (2026-09-15) — superseded in part by ADR-0032.** Tab and Workspace no longer carry one mixed icon. They carry an Agent rollup (agent-mode PTYs only) and a Terminal rollup (shell-mode PTYs only, including Working). The asymmetric rule below no longer applies; the rest of this ADR stands.
